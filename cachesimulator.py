...
#convert hex to binary and then split binary bits to get offset, set, tag bits
    ...
    print(f"ram_address:{address}")
    print(f"data:{data}")
    print("dirty_bit:") # what is this supposed to print the bit of? ----> should return the dirty bit for that line in cache
    
def cache_flush():
    if (write_hit == 2): #if write back then update ram for every cache line that has dirty bit = 1
        for i in range (0, num_lines):
            if (cache[i][1] == 1): #cache lines with a dirty bit, then update ram
                dataArray[i] = data.upper().lstrip("0x")
    else: #if cache flush, every line becomes valid bits all to 0 again
        for i in range(0, num_lines):
            cache[i][0] = 0
    np.delete(cache)
    print("cache_cleared")
